...
data2 <- read.table(text = gsub(",", "\t", readLines(fileToRead)), fill=TRUE) #This works sort of...
...
#pb = txtProgressBar(min = 0, max = 10000, initial = 0, style = 3) 


## UPDATE CODE AFTER FINISHING - data.table::rbindlist() is 10x faster than rbind
## OR DPLYR AND BIND_ROWS
## The problem is that rbind slows down the larger the file becomes
...
#for (i in 1:10000){
  ...
  for (k in 1:12){
        ...
        df <- data.table::rbindlist(df, dfRow)
  }
  curryear <- curryear+1
  if ( i %% 1000 == 0) {
